...
std::vector<int> PmergeMe::sortingVector(std::vector<int> data)
{
    if (data.size() <= 1)
        return data;
    ...
    if (!buildPairsVec(pairs, data, hasStraggler, straggler))
        return data;
    ...
    winnersTmp = winners;
    winnerSorted = sortingVector(winnersTmp);

    std::vector<bool> used(winners.size(), false);

    std::vector<int> reorderLoser(winnerSorted.size());
    ...
    data.clear();
    data = winnerSorted;
    insertLosersVec(data, reorderLoser, hasStraggler, straggler);
